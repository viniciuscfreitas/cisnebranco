// MRR calculations removed - not applicable for pet shop services
...
function calculateDashboardMetrics() {
  ...
  const projectCounts = { discoveryCount: 0, agreementCount: 0, buildCount: 0, serviceCompletedCount: 0, liveCount: 0 };
  const paidTasks = [];
  const pendingTasks = [];
  ...
  tasks.forEach(task => {
    ...
    if (colId >= 0 && colId <= 4) {
      const countKeys = ['discoveryCount', 'agreementCount', 'buildCount', 'serviceCompletedCount', 'liveCount'];
  ...
  return {
    ...
    serviceCompletedCount: projectCounts.serviceCompletedCount,
...
async function generateRecentActivities(tasks, useCache = true) {
  ...
  selectedTasks.slice(0, 3).forEach(task => {
    ...
    if (task.col_id === 0) {
      ...
      icon = 'fa-code';
    } else if (task.col_id === 4) {
      activityText = `<strong>${escapedClient}</strong> entregue`;
