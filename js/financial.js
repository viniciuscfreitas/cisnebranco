...
function updateFinancialRow(updatedTask) {
  ...
  if (financialSearchState.cachedMetrics) {
    ...
    if (oldTask) {
      ...
      if (oldIsPaid !== newIsPaid || oldIsPending !== newIsPending || oldPrice !== newPrice) {
    ...
    if (needsFullRecalc) {
      ...
      if (summaryCards.length >= 4) {
        ...
        if (statValue0) statValue0.textContent = formatCurrency(financialMetrics.currentMonthRevenue || 0);
        if (statChange0) statChange0.textContent = `Receita do mês`;
        ...
        if (statChange3) statChange3.textContent = `${financialMetrics.pendingCount} agendamentos`;
      }
    }
  }
}
    ...
    <div class="financial-grid">
      <div class="financial-summary">
        <div class="stat-card">
          ...
          <div class="stat-card-value" style="color: var(--success); font-size: 1.75rem;">${formatCurrency(financialMetrics.currentMonthRevenue || 0)}</div>
          <div class="stat-card-change">
            <span>Receita do mês atual</span>
          </div>
        </div>
...
function renderFinancialHeader(metrics) {
  ...
  DOM.headerInfo.innerHTML = `
    <div class="header-stat">
      <span class="header-stat-label">Receita Mensal</span>
      <span class="header-stat-value" style="color: var(--success);">${formatCurrency(metrics.monthlyRevenue || 0)}</span>
    </div>
    <div class="header-stat">
...
function exportFinancialData() {
  ...
  if (!tasksToExport || tasksToExport.length === 0) {
    NotificationManager.warning('Nenhum agendamento para exportar');
    return;
  }
  ...
  const csv = [
    'Métrica,Valor',
    `Receita Mensal,€${(metrics.monthlyRevenue || 0).toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`,
