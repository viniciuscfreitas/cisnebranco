...
function saveFormState() {
  ...
  const formState = {
    ...
    petName: (DOM.formPetName?.value || '').trim(),
...
function restoreFormState() {
  try {
    ...
    if (DOM.formPetName) DOM.formPetName.value = formState.petName || '';
    if (DOM.formContact) DOM.formContact.value = formState.contact || '';
    if (DOM.formType) DOM.formType.value = formState.type || 'Banho';
...
function resetFormToDefaults() {
  ...
  const formFields = [
    ...
    { el: DOM.formPetName, prop: 'value', val: '' },
    { el: DOM.formContact, prop: 'value', val: '' },
    { el: DOM.formType, prop: 'value', val: 'Banho' },
...
function openModal(task = null) {
  ...
  if (isEditingExistingTask) {
    ...
    DOM.formPetName.value = task?.pet_name || '';
    DOM.formContact.value = task?.contact || '';
    DOM.formType.value = task?.type || 'Banho';
    ...
    if (DOM.modalPreview) {
      const previewParts = [];
      if (task?.pet_name) {
        previewParts.push(`Pet: ${task.pet_name}`);
      } else if (task?.client) {
        previewParts.push(`Tutor: ${task.client}`);
      }
...
const FIELD_ERROR_MAP = {
  ...
  petName: { error: () => DOM.errorPetName, input: () => DOM.formPetName },
...
function validateFieldInline(fieldId, value) {
  ...
  switch (fieldId) {
    case 'client':
      if (!trimmedValue) {
        showFormError('client', 'Nome do tutor é obrigatório');
        isValid = false;
      } else {
        clearFormError('client');
      }
    ...
    case 'petName':
      if (!trimmedValue) {
        showFormError('petName', 'Nome do pet é obrigatório');
        isValid = false;
      } else {
        clearFormError('petName');
      }
      break;
...
function setupInlineValidation() {
  ...
  if (DOM.formPetName && !inlineValidationHandlers.petName.blur) {
    const blurHandler = () => {
      validateFieldInline('petName', DOM.formPetName.value);
      updateFormProgress();
    };
    const inputHandler = () => {
      if (DOM.formPetName.classList.contains('error')) {
        validateFieldInline('petName', DOM.formPetName.value);
      }
      updateFormProgress();
    };
    DOM.formPetName.addEventListener('blur', blurHandler);
    DOM.formPetName.addEventListener('input', inputHandler);
    inlineValidationHandlers.petName.blur = blurHandler;
    inlineValidationHandlers.petName.input = inputHandler;
  }
...
function validateForm() {
  ...
  const petName = DOM.formPetName ? DOM.formPetName.value.trim() : '';
  if (!petName) {
    showFormError('petName', 'Nome do pet é obrigatório');
    isFormValid = false;
  }
...
async function saveForm() {
  ...
  if (!DOM.formClient || !DOM.formPetName || !DOM.formPrice || !DOM.formType) {
  ...
  try {
    ...
    const formData = {
      client: clientName,
      pet_name: DOM.formPetName ? DOM.formPetName.value.trim() : '',
