...
function createCardElement(task, isExpanded = false, now = null) {
  ...
  if (isExpanded) {
    const petNameHtml = task.pet_name ? `<div class="card-detail-item"><span class="card-detail-label">Pet:</span><span class="card-detail-value">${escapeHtml(task.pet_name)}</span></div>` : '';
    ...
    // Uptime badge removed - not applicable for pet shop
    ...
    el.innerHTML = `
      <div class="card-header">
        <h3 class="card-title">${escapeHtml(task.pet_name || task.client)}</h3>
        <div style="display: flex; gap: 0.5rem; align-items: center;">
          ${badgeHtml}
        </div>
      </div>
