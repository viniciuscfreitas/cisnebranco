...
// MRR values removed - not applicable for pet shop

// MRR card removed - not applicable for pet shop services
...
function exportDashboardData() {
  ...
  const liveTasks = tasks.filter(t => t.col_id === 4);
  ...
  const serviceCompletedCount = tasks.filter(t => t.col_id === 3).length;
  const liveCount = tasks.filter(t => t.col_id === 4).length;
  ...
  const csv = [
    ...
    `Agendamentos Urgentes,${metrics.urgentCount || 0}`,
