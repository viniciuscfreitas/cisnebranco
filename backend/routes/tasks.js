...
function createTasksRoutes(db, NODE_ENV, sanitizeString, io) {
  ...
  router.post('/', (req, res) => {
    try {
      ...
      if (!clientSanitized) {
        return res.status(400).json({ success: false, error: 'Nome do tutor é obrigatório' });
      }

      const petNameSanitized = sanitizeString(pet_name, 255);
      if (!petNameSanitized) {
        return res.status(400).json({ success: false, error: 'Nome do pet é obrigatório' });
      }
      ...
      // Domain validation removed - not applicable for pet shop
      ...
      function attemptInsert(retryCount = 0) {
        ...
        db.run(
          `INSERT INTO tasks (
            id, user_id, client, pet_name, contact, type, stack, domain, description,
  ...
  router.put('/:id', (req, res) => {
    try {
      ...
      db.get('SELECT * FROM tasks WHERE id = ?', [taskId], (err, existing) => {
        ...
        const colIdNum = parseInt(col_id);
        if (isNaN(colIdNum) || colIdNum < 0 || colIdNum > 4) {
          return res.status(400).json({ success: false, error: 'col_id deve ser entre 0 e 4' });
        }
        ...
        db.run(
          `UPDATE tasks SET
            client = ?, pet_name = ?, contact = ?, type = ?, stack = ?, domain = ?, description = ?,
            price = ?, payment_status = ?, deadline = ?, deadline_timestamp = ?, hosting = ?,
      ...
      db.get('SELECT * FROM tasks WHERE id = ?', [taskId], (err, task) => {
        ...
        db.run(
          ...
          function (err) {
            ...
            setImmediate(() => {
              const colNames = ['Não Chegaram', 'Aguardando', 'Em Atendimento', 'Serviço Concluído', 'Entregue'];

              if (task.col_id !== colIdNum) {
                const fromCol = colNames[task.col_id] || task.col_id;
                const toCol = colNames[colIdNum] || colIdNum;
                const actionDescription = `Moveu agendamento ${task.pet_name || task.client} de ${fromCol} para ${toCol}`;
            ...
            // Check if task was moved to col_id = 4 (Entregue) and is recurring
            if (colIdNum === 4 && task.is_recurring === 1) {
              ...
              db.run(
                `INSERT INTO tasks (
                  user_id, client, pet_name, contact, type, stack, domain, description,
                  price, payment_status, deadline, deadline_timestamp, hosting,
                  col_id, order_position, is_recurring, assets_link, public_uuid
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
                [
                  ...
                  task.hosting,
                  0, // New task in Não Chegaram column
