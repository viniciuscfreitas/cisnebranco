        ...
        db.serialize(() => {
          ...
          `, (err) => {
            ...
            db.all(`PRAGMA table_info(tasks)`, (err, columns) => {
              ...
              // Migration: pet_name (CRÍTICO para pet shop)
              if (!columnNames.includes('pet_name')) {
                db.run(`ALTER TABLE tasks ADD COLUMN pet_name TEXT`, (err) => {
                  if (err) {
                    console.error('Error adding pet_name column:', err);
                  } else {
                    console.log('Added pet_name column to tasks table');
                  }
                });
              }
...
// Uptime monitor removed - not applicable for pet shop
    ...
    app.get('/api/tasks/view/:uuid', (req, res) => {
      try {
        ...
        db.get('SELECT client, col_id, updated_at FROM tasks WHERE public_uuid = ?', [uuid], (err, task) => {
          ...
          // Calculate progress based on col_id: 0=0%, 1=25%, 2=50%, 3=75%, 4=100%
          const progress = Math.round((task.col_id / 4) * 100);
          const colName = ['Não Chegaram', 'Aguardando', 'Em Atendimento', 'Serviço Concluído', 'Entregue'][task.col_id] || 'Desconhecido';
